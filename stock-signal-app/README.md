...
| 銘柄詳細 | `/stocks/[ticker]` | 価格チャート（SMA5/25/200 + BB±2σ）・出来高・RSI、テクニカル状態、ファンダ 6 カテゴリ判定と警戒フラグ、決算サプライズ（EPS 予想 vs 実績 4 四半期）、手動メモ（国別売上・事業構成）+ **AI 下書き生成**（SEC の年次報告書から自動抽出。要 `ANTHROPIC_API_KEY`、未設定でも他機能は動作） |
